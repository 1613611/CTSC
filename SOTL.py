...
if args.heavy_traffic:
    ...
    LOG_QUEUE_LENGTH_FILE_NAME = './log/%s/heavy-traffic/SOTL/queue-length.txt' % args.net_file
    LOG_VEHICLE_FILE_NAME = './log/%s/heavy-traffic/SOTL/vehicle.txt' % args.net_file
    LOG_TRAFFIC_LIGHT_FILE_NAME = './log/%s/heavy-traffic/SOTL/traffic-light.txt' % args.net_file
elif args.light_traffic:
    sumoCmd.extend(['-r', './network/%s.light.route.xml' % args.net_file])
    LOG_QUEUE_LENGTH_FILE_NAME = './log/%s/light-traffic/SOTL/queue-length.txt' % args.net_file
    LOG_VEHICLE_FILE_NAME = './log/%s/light-traffic/SOTL/vehicle.txt' % args.net_file
    LOG_TRAFFIC_LIGHT_FILE_NAME = './log/%s/light-traffic/SOTL/traffic-light.txt' % args.net_file
...
class Simulation_SOTL():
    ...
    def isFinised(self):
        if traci.simulation.getMinExpectedNumber() <= 0:
            ...
            t.add_row(['Arterial Travel Time', np.mean([veh['travel_time'] for _, veh in self.vehicle_tracker.items() if veh['routeID'] in ['route1', 'route2', 'route1A5A', 'route5B1B']])])
            t.add_row(['Collector Travel Time', np.mean([veh['travel_time'] for _, veh in self.vehicle_tracker.items() if veh['routeID'] not in ['route1', 'route2', 'route1A5A', 'route5B1B']])])
